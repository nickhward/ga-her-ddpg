...
for f in csv_files:
    ...
    df = pd.read_csv(f)
    print(df)
    length = len(df['test/reward'])
    print(length)
    for i in range(epochs):
        averages[i] += df['test/reward'][i]
        epoch_list[i] = df['test/episode'][i]

final_average_list = averages/20
...
for f in csv_files2:
    df = pd.read_csv(f)
    for i in range(epochs):
        averages2[i] += df['test/reward'][i]
    
final_average_list2 = averages2/20
