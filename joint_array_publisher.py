...
class JointArrayPub(object):
    def __init__(self):
        ...
        self.pos_pub = rospy.Publisher('/aubo_bot/target_pos', JointTrajectoryPoint, queue_size=10)
    ...
    def pub_pos_to_moveit(self, pos_array):
        x = JointTrajectoryPoint()
        x.positions = pos_array
        self.pos_pub.publish(x)


    def pub_joints_to_moveit(self, joints_array):
        self.check_publishers_connection()
        new_array = np.append(joints_array, [1.57, 0.0])
