...
def train(policy, rollout_worker, evaluator,
    ...
    for epoch in range(n_epochs):
        ...
        #rollout_worker.reset_all_rollouts()
        ...
        if total_success >= 3: #0.85
            ...
            if is_not_done:
                ...
                with open('slide_time.csv', 'a', encoding='UTF8', newline= '') as f:
                    writer = csv.writer(f)
                    writer.writerow(row_list1)
                is_not_done = False
...
def launch(
    ...
    return epochs
'''
Epochs taken to converge : 6
Tau = 0.924
Gamma = 0.988
Q_learning = 0.001
pi_learning = 0.001
random_epsilon = 0.912
noise_epsilon = 0.748
...
Q_learning = 0.001
pi_learning = 0.001
random_epsilon = 0.584
noise_epsilon = 0.232


 default=0.88, help='polyak averaging coefficient - Tau')
@click.option('--gamma_value', type=float, default=0.184, help='gamma - discounting factor')
...
@click.option('--random_epsilon', type=float, default=0.055, help='percentage of time a random action is taken')
@click.option('--noise_epsilon', type=float, default=0.774, help='std of gaussian noise added to not-completely-random actions as a percentage of max_u')
def main(**kwargs):

#######actual values being used for paper##################
    --polyak_value', type=float, default=0.484, help='polyak averaging coefficient - Tau')
@click.option('--gamma_value', type=float, default=0.928, help='gamma - discounting factor')
@click.option('--q_learning', type=float, default=0.001, help='critic learning rate')
@click.option('--pi_learning', type=float, default=0.001, help='actor learning rate')
@click.option('--random_epsilon', type=float, default=0.1, help='percentage of time a random action is taken')
@click.option('--noise_epsilon', type=float, default=0.597, help='std of gaussian noise 
'''


@click.command()
@click.option('--env', type=str, default='FetchSlide-v1', help='the name of the OpenAI Gym environment that you want to train on')
...
@click.option('--n_epochs', type=int, default=60, help='the number of training epochs to run')
...
@click.option('--polyak_value', type=float, default=0.484, help='polyak averaging coefficient - Tau')
@click.option('--gamma_value', type=float, default=0.928, help='gamma - discounting factor')
@click.option('--q_learning', type=float, default=0.001, help='critic learning rate')
@click.option('--pi_learning', type=float, default=0.001, help='actor learning rate')
@click.option('--random_epsilon', type=float, default=0.1, help='percentage of time a random action is taken')
@click.option('--noise_epsilon', type=float, default=0.597, help='std of gaussian noise added to not-completely-random actions as a percentage of max_u')
def main(**kwargs):
    launch(**kwargs)
