...
def fitness_function(genome):
    ...
    if gamma > 1:
        ...
        gamma = 0.5
    Q_lr = decode_small_function(genome[22:33])
    if Q_lr > 0.001:
        Q_lr = 0.001
    pi_lr = decode_small_function(genome[34:44])
    if pi_lr > 0.001:
        pi_lr = 0.001
    ...
    if epochs < bestepochs:
        ...
        with open('BestParameters.txt', 'a') as output:
            output.write("Epochs taken to converge : " + str(bestepochs - 16) + "\n")
    ...
    return 1/epochs
def decode_small_function(genome_partial):
