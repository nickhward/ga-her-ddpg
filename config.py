...
DEFAULT_PARAMS = {
    ...
    # training
    'n_cycles': 40, #50  # per epoch
    'rollout_batch_size': 2,  # per mpi thread
    'n_batches': 25,  # training batches per cycle
    'batch_size': 256,  # per mpi thread, measured in transitions and reduced to even multiple of chunk_length.
    'n_test_rollouts': 40, #50  # number of test rollouts per epoch, each consists of rollout_batch_size rollouts
...
def prepare_params(kwargs):
    ...
    #print(tmp_env.max_episode_steps)
    assert hasattr(tmp_env.spec, 'max_episode_steps')
    kwargs['T'] = tmp_env.spec.max_episode_steps
    tmp_env.reset()
