...
group = moveit_commander.MoveGroupCommander("manipulator_i5") #define the planning group (from the moveit packet 'manipulator' planning group)
...
box_name = "box"
scene.add_box(box_name, box_pose, size=(1.1, 1.1, 1.1))
print('adding box')
...
def pos_callback(data):
    pos = data.positions
    pub = rospy.Publisher('/aubo/movement_complete', Bool, queue_size=10)
    complete_msg = Bool()
    complete_msg.data = False
    check_publishers_connection(pub)
    pub.publish(complete_msg)

    assign_pos_values(pos[0], pos[1], pos[2], pos[3], pos[4],pos[5],pos[6])
...
def joint_callback(data):
    ...
    assign_joint_value(pos[0], pos[1], pos[2], pos[3], pos[4],pos[5])
...
def assign_pos_values(posx, posy, posz, rotx, roty, rotz, rotw):
    pose_goal = geometry_msgs.msg.Pose()
    pose_goal.position.x = posx
    pose_goal.position.y = posy
    pose_goal.position.z = posz
    pose_goal.orientation.x = rotx
    pose_goal.orientation.y = roty
    pose_goal.orientation.z = rotz
    pose_goal.orientation.w = rotw
...
def relative_joint_value(joint_0, joint_1, joint_2, joint_3):
    ...
    group_variable_values.pre_grasp_approach.direction.header.frame_id = "base_link";
  
    group_variable_values.pre_grasp_approach.direction.vector.z = -1.0;
    group.set_joint_value_target(group_variable_values) #set target joint values for 'manipulator' group
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/pickbot/target_joint_positions/', JointState, joint_callback)
    rospy.Subscriber('/pickbot/relative_joint_positions', JointState, relative_joint_callback)
    rospy.Subscriber('/aubo_bot/target_pos/', JointTrajectoryPoint, pos_callback)
