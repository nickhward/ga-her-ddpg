...
class PickbotEnv(gym.GoalEnv):
    ...
    def __init__(self, joint_increment=None, sim_time_factor=0.005, random_object=False, random_position=False,
        ...
        self._list_of_observations = ["distance_gripper_to_object",
                                      "shoulder_joint_state",
                                      "foreArm_joint_state",
                                      "upperArm_joint_state",
                                      "wrist_1_joint_state",
                                      "wrist_2_joint_state",
                                      "wrist_3_joint_state",
                                      "object_pos_x",
        ...
        rospy.Subscriber("/curr_pose", PoseStamped, self.current_pose_moveit_callback)
        ...
        #self.action_space = spaces.Box(np.array([-2,-2,-2,-2,-2,-2]), np.array([2, 2, 2, 2, 2, 2]), dtype="float32")
        ...
        if self._use_object_type:
            ...
            low = np.append(low, 0)
        #self.goal = np.array([-1.58, 1.2, -1.4, -1.87, 1.5, 0.0])
        #self.goal = np.array([-2.28, -1.21, -0.53, 0.67, 2.29, 0.0003])
        ...
        rot = Rotation.from_euler('xyz', [0.002567, 0.102, 1.563], degrees=True)
        ...
        print("--------------------------get distance to gripper and object------------------------------")
    ...
    def current_pose_moveit_callback(self, msg):
        self.current_pose_moveit = msg
    ...
    def reset(self):
        ...
        return self.get_obs()
        #return np.array(init_joint_pos)
        

    
    def get_distance_gripper_to_object(self, height=None):
        Object = np.array((0.3, -0.2, 0.4) if height is None else (0.4 + height/2))
        
        Gripper = np.array((self.current_pose_moveit.pose.position.x,
                            self.current_pose_moveit.pose.position.y,
                            self.current_pose_moveit.pose.position.z))
    ...
    def step(self, action):
        ...
        if self._joint_increment is None:
            ...
            next_action_position = self.get_action_to_position(action, self.curr_joint)
        ...
        new_obs = new_observation['observation']
        if new_obs[0] < self.min_distace:
            self.min_distace = new_obs[0]
        ...
        info = {}
        print("achieved--------------------------goal---------------------------")
        print(new_observation['achieved_goal'])
        print(self.goal)
        print("-------------------------------------------------------")
        goal = self.goal.copy()
        reward = self.compute_reward(new_observation['achieved_goal'], goal, info)
        self.accumulated_episode_reward += reward
        print("======================the reward==========================")
        print(reward)
        print('==========================================================')
    ...
    def set_target_object(self, random_object=False, random_position=False):
        ...
        if random_object:
            ...
            rot = Rotation.from_euler('xyz', [init_pos[3], init_pos[4], init_pos[5]], degrees=True)
    ...
    def get_obs(self):
        ...
        print("heeellllllllllllllllooooooooooooooooooooooooooooooooooooooooooooooooooo")
        print(joint_states)
        ...
        obs = np.concatenate([self.curr_joint, object])
        ...
        for obs_name in self._list_of_observations:
            ...
            elif obs_name == "shoulder_joint_state":
                observation.append(shoulder_joint_state)
            elif obs_name == "foreArm_joint_state":
                observation.append(foreArm_joint_state)
            elif obs_name == "upperArm_joint_state":
                observation.append(upperArm_joint_state)
            elif obs_name == "wrist_1_joint_state":
                observation.append(wrist1_joint_state)
            elif obs_name == "wrist_2_joint_state":
                observation.append(wrist2_joint_state)
            elif obs_name == "wrist_3_joint_state":
                observation.append(wrist3_joint_state)
    ...
    def is_done(self, observations):
        ...
        #if observations[7] != 0 and observations[8] != 0:
         #   done = True
        ...
        #if observations[7] != 0 or observations[8] != 0:
         #   done = True
