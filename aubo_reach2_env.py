...
class PickbotEnv(gym.GoalEnv):
    ...
    def __init__(self, joint_increment=None, sim_time_factor=0.005, random_object=False, random_position=False,
        ...
        self.movement_complete_pos = Bool()
        self.movement_complete_pos.data = False

        self.moveit_action_feedback = MoveGroupActionFeedback()
        self.feedback_list = []

        self.new_action = np.array([0.1,0.1,0.1,0.1])
        ...
        rospy.Subscriber('/aubo/movement_complete', Bool, self.movement_complete_callback_pos)
        ...
        #self.init_pos = np.array([0., 0., 0., 0., 0.])
        self.init_pos = np.array([0.131,0.377,1.834,1.457])
        #self.init_pos = np.array([0.247, 1.71,2.168,0.45])
       #self.init_pos = np.array([1.31, -1.12, -1.08, -0.99979112, 0.0000017842, 0.0041709607, 0.020007658])
        
        self.goal = np.array([-1.41, -1.012, 1.006, 0.449])
        #self.goal = np.array([-1.41, -.92, 1.07,1.55])
        #self.goal = np.array([0.0071, -.798, 0.589])

        #self.goal = np.array([0.612,1.3566,-1.234,0.4995,1.3])
        #self.goal = np.array([0.612,1.3566,-1.234,0.4995,1.3])
        self.publisher_to_moveit_object.pub_joints_to_moveit(self.init_pos)
    ...
    def random_init_joints(self):
        random_joint = np.zeros(4)
        for i in range(len(random_joint)):
            random_joint[i] = np.random.default_rng().uniform(low=-1.7, high=1.7)
        
        return random_joint

    def random_goal(self):
        goal_joints = np.zeros(4)
    ...
    def movement_complete_callback_pos(self, msg):
        self.movement_complete_pos = msg
    ...
    def reset(self):
        ...
        #init_pos = self.random_init_joints()
    ...
    def step(self, action):
        ...
        while True:
            ...
            if np.linalg.norm(self.init_pos - obs["achieved_goal"], axis=-1) < 0.01:
                break
            elif elapsed_time > rospy.Duration(3):
                break
        self.count = 1
        print('action: {}'.format(self.init_pos))
    ...
    def _get_obs(self):
        ...
        shoulder_joint_state_action = self.new_action[0]
        foreArm_joint_state_action = self.new_action[1]
        upperArm_joint_state_action = self.new_action[2]
        wrist_joint_action = self.new_action[3]
       #wrist2_joint_action = self.new_action[4]
        curr_action = np.array([shoulder_joint_state_action,foreArm_joint_state_action,upperArm_joint_state_action,wrist_joint_action])
        ...
        curr_action = np.array([posx, posy, posz])
        '''
            #print("==================================================================================")
        ...
        wrist_joint_state = joint_states.possition[3]
        wrist2_joint_state = joint_states.position[4]
        '''
        counter = 0
        #new_action_joints = self.new_action
        self.movement_complete.data = False      
        increment_joints = curr_action*0.05
        #for i in increment_joints:
        for i in curr_action:
            self.init_pos[counter] += increment_joints[counter]
            counter += 1
            
        #rot = np.array([1.,0.,1.,0.])
        #final_pos = np.concatenate(self.init_pos, rot)
        ...
        #current_joint = np.asarray([self.joints_state.position[0], self.joints_state.position[1], self.joints_state.position[2], self.joints_state.position[3]])
        ...
        #self.curr_joint = np.array([shoulder_joint_state, foreArm_joint_state, upperArm_joint_state, wrist_joint_state, wrist2_joint_state])
        #self.curr_joint = self.curr_joint

        object = self.goal
        #rel_pos = self.get_distance_gripper_to_object(self.joints_state.position)
        
        achieved_goal = self.init_pos.copy()
